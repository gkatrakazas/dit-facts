...
const PassedCoursesSelection = ({ availableCourses, selectedCourses, setSelectedCourses, onClose, filterType, setFilterType }) => {
  ...
  useEffect(() => {
    if (selectionMode === "multi") {
      setTempSelectedYears(selectedCourses);
    } else if (selectionMode === "range" && selectedCourses.length) {
      setRange({ start: selectedCourses[0], end: selectedCourses[selectedCourses.length - 1] });
    }
  ...
  const handleModeChange = (mode) => {
    ...
    setTempSelectedYears(selectedCourses);
    if (mode === "range" && selectedCourses.length) {
      setRange({ start: selectedCourses[0], end: selectedCourses[selectedCourses.length - 1] });
    } else {
      setRange({ start: "", end: "" });
    }
  ...
  const applyFilter = () => {
    if (selectionMode === "multi") {
      setSelectedCourses([...tempSelectedYears]); // Ensure new array reference
    } else if (selectionMode === "range" && range.start && range.end) {
      const filteredYears = availableCourses.filter(
        (year) => year >= range.start && year <= range.end
      );
      setSelectedCourses([...filteredYears]); // Ensure new array reference
    }
...
export default PassedCoursesSelection;
